...
router.get('/search', withAuth, async (req, res) => {
  ...
  try {
    // Primeiro find(): Pesquisa todas as notas que são do autor
    // Segundo find(): Todos os campos que estão indexados com text
  ...
  try {
    ...
    if (isOwner(req.user, note)) {
      let note = await Note.findOneAndUpdate(
        ...
        // Devolver a nova nota, e não a antiga
        { upsert: true, new: true }
      );
      // Devolução da nota atualizada
      res.json(note);
    } else {
